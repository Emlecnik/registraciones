...
async function cargarRevisar(){
  ...
  totales.forEach(item => {
    ...
    li.innerHTML = `
      ...
      <button class="btn-exportar" data-mes="${item.mes}">EXPORTAR</button>
    `;
    ul.appendChild(li);
  });

  ul.querySelectorAll('.btn-exportar').forEach(btn => {
    btn.addEventListener('click', () => exportarMes(btn.dataset.mes, data[btn.dataset.mes]));
  });
...
/* =========================================================
   ...
   ========================================================= */
async function exportarMes(mesKey, monthData){
  const totalTransf = (monthData.transferencias || []).reduce((a, b) => a + b, 0);
  const totalVeinte = (monthData.veinte || []).reduce((a, b) => a + b, 0);
  const total = totalTransf + totalVeinte;

  document.getElementById('export-mes-titulo').textContent = mesKey.replace('-', ' ');
  document.getElementById('export-transf').textContent = formatARS(totalTransf);
  ...
  document.getElementById('export-total').textContent = formatARS(total);

  const tarjeta = document.getElementById('tarjeta-exportar');
  tarjeta.style.left = '0px'; // mover a la vista temporalmente para capturar bien
  tarjeta.style.zIndex = '-1';

  const formato = confirm('Aceptar = descargar como PDF.\nCancelar = descargar como imagen PNG.') ? 'pdf' : 'png';
